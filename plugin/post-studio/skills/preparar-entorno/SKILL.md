...
# preparar-entorno — preflight y bootstrap de post-studio
...
- `timezone`: zona horaria real del usuario para programar publicaciones (ej. `America/Costa_Rica`) — si la plataforma de publicación no la tiene en su lista (pasa seguido), `publicar-contenido` ya sabe buscar el offset UTC equivalente más cercano.
...
- **SVG**: se puede usar directo en el template HTML (los navegadores lo renderizan nativo). Para el watermark de video (ffmpeg no decodifica SVG), avisale a `crear-reel` que necesita una versión PNG rasterizada — ese skill la genera una sola vez y la cachea (ver su paso de marca de agua).

Guardá el nombre de archivo final elegido en `brand.config.json` → `logoFile` (y `logoAspect` si ya lo mediste, ver `crear-imagen`) — los demás skills leen ese campo en vez de volver a buscar o de asumir un nombre de archivo fijo.
...
## 5. Conectores MCP
Solo aplica cuando el usuario va a publicar (`publicar-contenido`). Intentá resolver las tools del navegador Chrome real:
