...
Si no corriste `preparar-entorno` en esta sesión (o `brand.config.json` no existe todavía en la raíz del proyecto), corrélo primero — de ahí sale `contentRoot`, el logo resuelto, y el texto de marca (`website`) que usa el paso 13. Además, confirmá vos mismo acá lo específico de este skill (no lo cubre `preparar-entorno` por defecto):
```bash
ffmpeg -version
ffprobe -version
```
...
Si el video de origen no es ~9:16, usá el script de esta skill (fondo desenfocado en vez de barras negras, consistente con el layout `full_bleed` de `crear-imagen`):
```bash
bash "${CLAUDE_PLUGIN_ROOT}/skills/crear-reel/scripts/format_vertical.sh" "raw.mp4" "vertical.mp4"
```
...
```bash
bash "${CLAUDE_PLUGIN_ROOT}/skills/crear-reel/scripts/apply_watermark.sh" \
